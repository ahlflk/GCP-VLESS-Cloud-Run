...
auto_deployment_setup() {
    ...
    progress_bar "Enabling APIs" 5 # Time: 5s (Increased for accuracy)
...
clone_and_extract() {
    ...
    progress_bar "Cloning Repository" 3 # Time: 3s (Adjusted)
...
prepare_config_files() {
    ...
    progress_bar "Preparing Config" 1 # Time: 1s
}
...
deploy_to_cloud_run() {
    ...
    gcloud run deploy $SERVICE_NAME \
      ...
      --cpu $CPU \
      --quiet > /dev/null 2>&1
    progress_bar "Deploying Service" 8 # Time: 8s (Increased for accuracy)
